...
# Startup safety invariants (§28.2 rule #10, §28.21, §28.23) live in
# ``app/agent/invariants.py`` — a single source of truth shared with the
# Phase 11 FastAPI sidecar (§31.3) so the native app enforces the exact
# same tool-registry guarantees as ``streamlit run``.
