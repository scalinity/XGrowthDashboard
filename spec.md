...
1. **Never post or reply to X without explicit per-action confirmation.** Drafts always land first. Publishing requires Daniel to click a confirmation button for each individual post — no batch approvals, no auto-publish. Agent Chat can prepare the exact final text and guide Daniel to the owner-confirmed publish workflow; it must not present this as a dead end. The agent itself never invokes `publish_post_to_x` or `publish_reply_to_x` from chat. Those internal tools run only after the UI has captured Daniel's confirmation click and passed a valid single-use `confirmation_token` to the tool layer.
...
15. **Niche must be defined before saving drafts — orchestrator-gated.** The agent refuses to call `save_draft_post` or `save_draft_reply` when either `niche_problem` or `niche_person` setting is empty. This blocks persistence, not help: the agent may still draft inline in chat, clearly mark the text as unsaved, and name Settings → Growth Agent → Niche as the step needed before saving. The refusal happens at the orchestrator level (`app/agent/session.py`), NOT in the agent's prompt — Daniel cannot tell the agent to "skip the niche check," and a prompt-injected reply target cannot bypass it. The reason this is a non-negotiable persistence rule rather than a soft warning: saved drafts are durable strategy artifacts, and niche identity is the load-bearing assertion that ties voice, content type, and reply targets to a coherent strategy. See §28.16 for the Settings panel + system-prompt splice details.
...
- **Atomicity (Phase 8 R-1 split-txn pattern, post-/review-2 P8R-1):** the X API subprocess call runs OUTSIDE any open SQLite transaction so the writer-lock is never held across network I/O (Phase 7 added concurrent launchd jobs that would otherwise SQLITE_BUSY for ~90s during retries; `_log_raw` audit rows for failed attempts would also roll back with the publish txn). The flow is: (1) read-only preflight (no txn) — length cap, rate capacity, reply-target resolution, AND read-only token validation via `confirmation.validate_token_only` so an invalid token never burns an X API call; (2) X API call OUTSIDE any txn with bounded internal retry (see below) — `_log_raw` rows commit in autocommit and SURVIVE if the downstream commit fails; (3) commit txn (BEGIN IMMEDIATE held for milliseconds only) — validate-and-consume token + set `published_to_x_at`, `x_post_id`, `publish_method = agent_confirmed`, `manual_confirmation_status = confirmed`, clear `publish_last_error`, increment `publish_attempt_count`, mark `consumed_at_utc`, update `posts.published_via_agent_message_id`, set `publish_confirmation_tokens.consumed_by_x_post_id` to the X API response's `data.id` → COMMIT. On any DB error after the X API succeeded (`api_data is not None`) → set `publish_method = 'unknown'` + `published_to_x_at` so the §28.10 step 8 crash-recovery scan picks the row up via the existing orphan-detection predicate; never roll back the conceptual fact that a post is live on X. The token-consumed matrix (429 UN-consumed; 403 / 5xx-after-retry / timeout all CONSUMED via explicit narrow-txn `_emit_api_failure(consume_token=True, ...)`) is preserved verbatim. Token-consume + posts UPDATE remain atomic *with each other*; the X API call is no longer atomic *with* the DB writes (which was never achievable — X is an external system outside our transaction control). See `app/agent/publish.py::publish_post_atomic` for the canonical implementation.
...
6. Tool layer (Phase 8 R-1 split-txn pattern, post-/review-2 P8R-1): (a) read-only preflight — fetch the `posts` row, length-cap check, rate-capacity gate (API branch only), reply-target resolution, AND read-only token validation via `confirmation.validate_token_only` so an invalid token never burns an X API request. Any preflight failure → return error to click-handler without opening a transaction; token stays unconsumed if its expiry allows (Daniel can fix and retry). (b) X API call OUTSIDE any txn — `POST /2/tweets` with bounded retry (`x_posting_publish_retry_attempts_per_token` times on 5xx). NO retry on 429 / 403 / timeout (preserves token semantics and avoids duplicate-post on timeout). `_log_raw` audit rows commit in autocommit and survive a downstream commit failure. (c) Commit txn — BEGIN IMMEDIATE → validate-and-consume token (same six checks re-run atomically) → set `posts.published_to_x_at`, `x_post_id`, `publish_method = agent_confirmed`, `manual_confirmation_status = confirmed`, clear `publish_last_error`, increment `publish_attempt_count`, set `publish_confirmation_tokens.consumed_at_utc` AND `consumed_by_x_post_id`, insert `agent_messages` row, update `posts.published_via_agent_message_id`, redact the raw token from the `agent_tool_calls.arguments_json`, set `redacted_arguments = true` → COMMIT. Writer-lock held only for milliseconds. → On X API failure paths: 429 → no txn, token UN-consumed, surface "rate-limited until reset"; 403 cold-reply → narrow txn consumes token + `publish_method='failed'`, no posts row created, surface "engage with this author's posts first, or use the manual fallback"; 5xx after retry exhaustion → narrow txn consumes token (rule #10(f)) + `publish_method='failed'`; timeout mid-call → narrow txn consumes token + `publish_method='unknown'` + `published_to_x_at` set so the §28.10 step 8 crash-recovery scan picks the row up (never retried — risk of duplicate post). All failure paths log to `agent_tool_calls` with `status = error` and write a `publish_failed_<kind>` audit row.
...
### API + Grok consolidation (2026-05-22) — §29.1 Phase 7/8/9, §29.12, §25 Phases 7/8/9

Phase 7, Phase 8, Phase 9. Three deferred capabilities — originally V1.1 (X API reads), V1.2 (X API writes), V1.2 (Grok firehose discovery) — are promoted to comprehensive v1 scope per the §0 sixth revision note, slotted as the three phases after Phase 6 Blogs shipped. Five new computed surfaces (four scheduled jobs in §17 + one Grok sweep), one schema-promotion (`reply_target_snapshots` from V1.1+ → v1), four new columns on `reply_targets` (`lint_category`, `force_drafted`, `force_drafted_reason`, plus the now-populated `velocity_score` / `timing_score`), one new audit table (`grok_api_responses`), one new audit-log category (`scheduled_job`), eight new settings rows across the three migrations, and one new full §29.12 section. Manual workflows remain inviolable Settings-selectable fallbacks for every API path. `audience_quality_score` 7th resolver dimension and `v_content_type_x_pillar_performance` cross-pivot stay deferred (density / data-source reasons, not API).

98. **Phase 7 — X API reads (migration 018).** xurl OAuth one-time setup; four scheduled read jobs (account snapshot daily, recent posts daily + one-shot backfill, post metrics hourly, reply-target metrics hourly per §17); `reply_target_snapshots` promoted to v1; `velocity_score` + `timing_score` activated (resolver: 6 dimensions, 4⁶=4096-combo pure-function test); §29.10 thread-classifier lint (Haiku) with `lint_blocked` + force-draft override + mandatory `force_drafted_reason`; 404 → `status='target_deleted'` detection; programmatic X API auto-pull added to §28.20 replier-pool, §28.24 Account Researcher, §28.25 Profile Audit (same xurl wrapper); `data_collection_mode` default flips to `'api'`; launchd plists ship documented but NOT auto-loaded; combined Anthropic + xAI cost ceiling (§28.6) at $30 default. Distinct from the §28.18 reply-quality lint that shipped with Phase 5.9 — Phase 7's lint categorizes the *target post's thread quality*; §28.18's lint categorizes the *draft Daniel writes*. (§10 `reply_target_snapshots`, §10 `posts.last_metrics_refresh_at_utc`, §10 `reply_targets.lint_category` / `.force_drafted` / `.force_drafted_reason`, §17 Phase 7 scheduling, §18 xurl OAuth + rate-limit notes, §22 X-API-failure edge cases, §25 Phase 7, §28.6 combined ceiling, §28.20 / §28.24 / §28.25 auto-pull, §28.30 `scheduled_job` audit category, §29.1 Phase 7 block, §29.2 source-of-truth invariant, §29.3 resolver interaction, §29.6 schema promotion + settings rows, §29.7 force-draft UI, §29.10 thread-classifier lint, §29.11 X-API rate-limit edge case)

99. **Phase 8 — X API writes (migration 019).** Real `POST /2/tweets` branch replaces the Phase-5.5-stubbed manual-clipboard-only branch in §28.10, alongside the existing manual branch — same six-check + atomic-transaction wrapper. New `publish_via_api_enabled` setting (default TRUE) gates the per-publish API-vs-manual branch; manual remains a Settings-selectable fallback forever. Write-side rate-limit handling (sliding window per `x_write_rate_limit_per_15min` + `x_write_rate_limit_per_24h`); 429 keeps token unconsumed; 403 on cold reply surfaces "engage first or use manual fallback" UX; timeout mid-transaction → ROLLBACK + §28.10 crash-recovery reconciliation. Sandbox testing uses vcr.py recorded fixtures (`scripts/rerecord_x_api_fixtures.py` documented). (§10 `publish_via_api_enabled` + write rate-limit settings, §22 X-API-write edge cases, §25 Phase 8, §28.10 Phase 5.5 → Phase 8 transition paragraph, §29.1 Phase 8 block, §29.6 settings rows, §29.7 Mark posted via §28.10, §29.11 cold-reply edge case)
